...
public sealed class FabricLakehouseService
{
    ...
    public async Task<bool> WriteFileAsync(string relativePath, string content)
    {
        ...
        {
            _logger.LogInformation("Fabric config is not set; skipping OneLake write for {Path}.", Sanitize(relativePath));
            return false;
        }
    ...
    public async Task<object> QueryStatusAsync()
    {
        ...
        try
        {
            ...
            return new { connected = response.IsSuccessStatusCode, statusCode = (int)response.StatusCode };
        }
        catch (Exception ex)
        {
